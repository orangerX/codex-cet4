...
export default function Home() {
 ...
 function move(delta:number){
  ...
  const nextIndex=(safeIndex+delta+list.length)%list.length;
  setIndex(nextIndex);
  setFlipped(false);
  setNotice('');
  speak(list[nextIndex]?.word);
 }
 ...
 function speak(targetWord=word?.word){if(!targetWord)return;if(!('speechSynthesis' in window)){setNotice('当前浏览器不支持发音，请换用支持语音朗读的浏览器。');return;}window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(targetWord);u.lang='en-US';u.rate=.8;u.onerror=()=>setNotice('发音暂不可用，请检查设备语音或稍后重试。');window.speechSynthesis.speak(u);}
 useEffect(()=>{
  function keyboard(e:KeyboardEvent){
   ...
   if(e.key==='ArrowUp'){
    e.preventDefault();
    if(!e.repeat&&word)setFlipped(f=>!f);
    return;
   }
 ...
 <div className="feedback" role="status">{notice||<span>先回忆，再翻卡，让记忆多停留一会儿。</span>}</div><div className="keyboard-help"><kbd>↑</kbd>翻转卡片<i/> <kbd>←</kbd><kbd>→</kbd>切换单词<i/><kbd>M</kbd>朗读单词</div></TabsContent></Tabs>
